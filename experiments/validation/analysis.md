...
**Sequential ANOVA (600 trials, all 5 models):**

| Factor | F | p | Significant? |
|--------|---|---|-------------|
| Model | 1.49 | 0.205 | No |
| N agents | 130.08 | <0.001 | Yes |
| Visibility | 286.44 | <0.001 | Yes |
| Temperature | 1.20 | 0.274 | No |
| Visibility x N agents | 1600.63 | <0.001 | Yes |

Model and temperature don't matter for sequential execution. All five models solve the visible condition in exactly 2 steps/agent (view, book) regardless of N. The hidden condition forces trial-and-error, and steps scale with N because each successive agent has more taken slots to collide with.
...
### 3. Concurrency Changes Everything

**Concurrent ANOVA (600 trials, all 5 models):**

| Factor | F | p | Significant? |
|--------|---|---|-------------|
| Model | 30.63 | <0.001 | Yes |
| N agents | 523.23 | <0.001 | Yes |
| Visibility | 163.07 | <0.001 | Yes |
| Temperature | 17.98 | <0.001 | Yes |
| Visibility x N agents | 429.49 | <0.001 | Yes |

Under concurrency, every factor matters. Model goes from F=1.49 (ns) to F=30.63 (p<0.001). Temperature goes from F=1.20 (ns) to F=17.98 (p<0.001). Concurrent execution is the condition that separates models.
...
|---|---|---|---|---|---|---|
| GPT-OSS | 3.93 | 2.00 | 4.52 | 2.96 | 7.74 | 5.23 |
| Mercury 2 | 3.60 | 2.00 | 4.30 | 2.84 | 6.62 | 4.52 |
| DeepSeek | 3.33 | 1.97 | 4.12 | 2.98 | 4.86 | 5.36 |
| Kimi | 3.67 | 2.00 | 4.48 | 2.92 | 5.86 | 5.05 |
| GLM-5 | 3.00 | 1.97 | 3.16 | 2.80 | 4.38 | 3.85 |
...
|-------|-----------|------------|-------------------|---------------|-------------------|
| GPT-OSS 120B | $0.15 | $0.60 | 3.95 | $0.005 | 27,230 |
...
GPT-OSS and Mercury 2 tie on $/trial (~$0.005) because of their low token prices. But the story diverges on steps: Mercury 2 needs 17% fewer steps than GPT-OSS (3.26 vs 3.95) while costing the same per trial. Under the hardest concurrent conditions (N=10, visible, t=0.0), the gaps widen:
...
GPT-OSS takes nearly twice the steps of GLM-5. Mercury 2, despite being a diffusion model optimized for throughput rather than reasoning depth, sits in the middle of the pack on steps but delivers them at 1,000+ tokens/second, making it the fastest wall-clock model by a wide margin.
...
**Mercury 2 is the best cost-latency tradeoff.** It matches GPT-OSS on $/trial while being 17% more step-efficient and dramatically faster in wall-clock time (diffusion architecture produces ~1,000 tokens/second). A Mercury 2 N=3 concurrent trial completes in 1.1-2.0 seconds. The same trial takes GPT-OSS 3-5 seconds and GLM-5 9-110+ seconds (due to API latency issues).
...
|---|------------|-----------|-------------|------------|
| 3 | 2.00 | 1.87-2.00 | 3.00-3.93 | 1.93-2.00 |
| 5 | 2.00 | 2.44-3.00 | 3.12-4.52 | 2.42-2.98 |
| 10 | 2.00 | 3.71-5.32 | 3.64-7.74 | 3.76-5.36 |
...
- Hidden sequential N=10: 0.74-0.81 (most attempts are wasted because agents can't see the calendar)
- Visible concurrent N=10: 0.24-0.35 (concurrent stale-read collisions waste some attempts)
...
- **Sequential**: no significant effect (F=1.60, p=0.206). At t=0.0, all visible conditions are deterministic (SD=0). At t=0.7, most visible conditions are still deterministic. Temperature introduces variance only in hidden conditions with large N.
- **Concurrent**: significant (F=20.30, p<0.001). Temperature mostly reduces steps (agents make slightly different choices, reducing herd behavior), but increases variance.
...
| Hidden, sequential | 1.87 - 5.32 | 0 violations | Scales with N. Model matters slightly. |
| Visible, concurrent | 3.00 - 7.74 | 0 violations | Visibility reversal. Stale reads cause thundering herd. |
| Hidden, concurrent | 1.93 - 5.36 | 0 violations | Similar to sequential hidden. Guard serialization preserves structure. |
...
But the real cost of extra steps in agentic workflows is latency, not tokens. Each step is a blocking LLM round-trip. An agent taking 8 steps at 500ms/call spends 4 seconds waiting. An agent taking 4 steps spends 2 seconds. When you have 10 concurrent agents and throughput matters, halving the step count halves the wall-clock time. Token cost differences between models are dwarfed by step-count differences:

- GPT-OSS at 7.74 steps/agent (N=10 concurrent visible) means 77% more round-trips than GLM-5 at 4.38
- Mercury 2 at 6.62 steps is mid-pack on steps, but its diffusion architecture delivers each step ~5x faster than autoregressive models, making it the fastest end-to-end despite more steps than GLM-5
...
| Mercury 2 | 3.26 | 1.30 | 39.1% | 4.15 |
| Kimi K2.5 | 3.47 | 1.29 | 41.7% | 21.35 |
| DeepSeek V3.2 | 3.79 | 1.61 | 43.0% | 54.89 |
| GPT-OSS 120B | 3.95 | 1.77 | 44.0% | 14.75 |

*GLM-5's high wall clock is due to API latency/timeouts, not model speed. 199 out of 681 Phase 2b records for GLM-5 were timeout errors.

Mercury 2's 4.15s average wall clock is notable. Its diffusion architecture produces tokens in parallel rather than autoregressively, yielding ~1,000 tokens/second throughput. For agentic workloads where each step is a blocking round-trip, this translates directly to lower end-to-end latency. A 10-agent concurrent trial that takes GPT-OSS 15-65 seconds completes on Mercury 2 in 2-20 seconds.
...
| Visible N=10 | 2.2-2.7 | 7-21 | 0% (R2) |
| Hidden-cal N=3 | 2.00 | 27-30 | 100% |
| Hidden-cal N=5 | 2.8-3.0 | 36-50 | 100% |
| Hidden-cal N=10 | 4.8-5.4 | 74-99 | 80-100% |
...
|-----------|--------------------------|---------------------------|
| Visible N=3 | 2.00 | 3.3-3.7 |
| Visible N=5 | 2.00 | 4.2-4.6 |
| Visible N=10 | 2.2-2.7 | 5.6-6.5 |
| Hidden-cal N=3 | 2.00 | 2.00 |
| Hidden-cal N=5 | 2.8-3.0 | 2.8-2.9 |
| Hidden-cal N=10 | 4.8-5.4 | 4.8-5.3 |
...
Double booking patterns are unchanged from sequential: hidden-calendar condition produces them (self-re-booking), visible-calendar does not (at N=3 and N=5).
