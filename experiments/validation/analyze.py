...
def run_anova(df: pd.DataFrame) -> None:
    """Run factorial ANOVA on steps_per_agent."""
    print("=" * 90)
    print("FACTORIAL ANOVA — Steps per Agent")
    print("=" * 90)

    # For sequential (pilot) data
    seq = df[df["execution_mode"] == "sequential"].copy()
    if len(seq) > 0:
        print("\n--- Sequential (Pilot) ---")
        _anova_block(seq)

    # For concurrent (phase2b) data
    conc = df[df["execution_mode"] == "concurrent"].copy()
    if len(conc) > 0:
        print("\n--- Concurrent (Phase 2b) ---")
        _anova_block(conc)


def _anova_block(df: pd.DataFrame) -> None:
    """Run one-way ANOVAs for each factor."""
    # Main effects
    factors = ["model", "n_agents", "visibility", "temperature"]
    if df["n_rounds"].nunique() > 1:
        factors.append("n_rounds")
    for factor in factors:
        groups = [g["steps_per_agent"].values for _, g in df.groupby(factor)]
        if len(groups) < 2:
            continue
        ...
        print(f"  {factor:<15} F={f_stat:8.2f}  p={p_val:.6f}  {sig}")

    # Key interaction: visibility × n_agents
    groups = []
    labels = []
    for (vis, n), g in df.groupby(["visibility", "n_agents"]):
        groups.append(g["steps_per_agent"].values)
        labels.append(f"{vis}_N{n}")
    if len(groups) >= 2:
        f_stat, p_val = stats.f_oneway(*groups)
        print(f"  {'vis×n_agents':<15} F={f_stat:8.2f}  p={p_val:.6f}")

    # Effect sizes (Cohen's d) for key comparisons
    print("\n  Effect sizes (Cohen's d):")
    vis = df[df["visibility"] == "visible"]["steps_per_agent"]
    hid = df[df["visibility"] == "hidden"]["steps_per_agent"]
    if len(vis) > 0 and len(hid) > 0:
        d = _cohens_d(vis, hid)
        print(f"    visible vs hidden:  d={d:.3f}")

    for n in sorted(df["n_agents"].unique()):
        sub = df[df["n_agents"] == n]
        v = sub[sub["visibility"] == "visible"]["steps_per_agent"]
        h = sub[sub["visibility"] == "hidden"]["steps_per_agent"]
        if len(v) > 0 and len(h) > 0:
            d = _cohens_d(v, h)
            print(f"    vis vs hid @ N={n}: d={d:.3f}")
