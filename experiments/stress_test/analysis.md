...
50 total shared room slots, 129 dept booking attempts (241 including retries) = 4.8:1 contention, the highest of any resource type. Sales has the highest success rate (35%) with targeted attempts. Product generates the most demand (35 attempts) and converts at only 6%.
...
Sales has no task scope. Four departments have 15 abstract work units each (`eng/1` through `eng/15`, etc.). These are generic claimable items used to test the FIRST_WRITER conflict policy and dependency gating under the CLASSIFIED visibility level. Only department members can see or claim their own tasks.
...
With FIRST_WRITER policy, the first agent to claim a task wins. Eng has the most attempts (34 for 15 tasks) because 20 eng agents each want 1-2 tasks, creating significant contention. The dependency chains serialize a small fraction of work.
...
|---|---|---|---|---|
| product | 28.1 | 12.8 | 54.4% | 6.9 |
| design | 28.0 | 11.1 | 60.1% | 7.4 |
| eng | 23.9 | 9.8 | 59.0% | 6.6 |
| sales | 23.9 | 10.7 | 55.3% | 6.4 |
| ops | 21.0 | 7.8 | 63.0% | 6.7 |

Design and product agents are the busiest (~28 steps) because they target both shared rooms and equipment/dept rooms. Product agents have the lowest efficiency (54.4%) because they target the most contested resources: shared rooms (35 attempts) and dept rooms (46 attempts). Ops agents are the most efficient (63.0%), targeting lower-contention resources.

Efficiency ranges from 54-63% across departments. The deferred parking/boardroom pattern counts every request as "wasted" at the tool level (BLOCKED), pulling efficiency down. Actual allocation happens in resolution, which isn't captured in per-tool efficiency.
...
| 1 | 415 | 521 | 22 | 2 | 4 |
| 2 | 806 | 1,007 | 47 | 4 | 8 |
| 3 | 1,223 | 1,523 | 79 | 6 | 13 |
| 4 | 1,801 | 2,266 | 130 | 9 | 18 |
| 5 | 2,398 | 3,039 | 193 | 12 | 24 |
| 6 | 3,139 | 3,997 | 265 | 16 | 30 |
| 7 | 3,908 | 5,000 | 355 | 20 | 37 |
| 8 | 4,820 | 6,185 | 464 | 25 | 44 |
| 9 | 5,747 | 7,397 | 590 | 30 | 51 |

Actions grow roughly linearly (~93/round). The intent-action gap widens over time (from 52 in round 0 to 285 by round 9) because deferred intents (parking + boardroom) accumulate across rounds. Each round adds new intents that are resolved but not directly converted by the agent. Adversarial agents also contribute to the gap through denied intents.
