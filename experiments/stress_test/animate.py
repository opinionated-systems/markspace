...
TOOL_ZONE = {
    "book_dept_room": 0, "book_shared_room": 0,
    "request_boardroom": 1,
    "request_parking": 2,
    "order_lunch": 3, "view_lunch_availability": 3,
    "claim_task": 4,
    "book_equipment": 5,
}
...
DAY_LABELS = {
    0: "MON AM", 1: "MON PM", 2: "TUE AM", 3: "TUE PM",
    4: "WED AM", 5: "WED PM", 6: "THU AM", 7: "THU PM",
    8: "FRI AM", 9: "FRI PM",
}
...
for s in steps_data:
    ...
    if name in agents:
        continue
    dept = next(
        (d for d in DEPT_ORDER
         if name.startswith(d) or name.startswith(f"adv-{d}")),
        "ops",
    )
...
def draw_agents(draw):
    for di, dept in enumerate(DEPT_ORDER):
        ...
        dept_active = any(
            n in agent_pulses for n in dept_agents[dept]
        )
        if dept_active:
...
def draw_footer(draw):
    ...
    for label, key in [("ALLOW", "allow"), ("BLOCKED", "blocked"),
