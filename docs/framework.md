...
| Content Policy | Pluggable content inspection pipeline for Observation marks. Policies can ALLOW, REWRITE (sanitize content), REJECT (block write), or FLAG (allow but warn). Evaluated after rate limits, before scope validation. | Guard `_check_content_policies()` + `ContentPolicy` ABC, [spec 9.13](spec.md#913-content-policies) |

<p align="center"><img src="../figures/single_agent.svg" alt="Single agent architecture" width="700"></p>

The first three checks (authority, validation, redundancy) are the **static enforcement layer** - they reject malformed or unauthorized writes at the boundary. These are non-negotiable: every deployment needs them. The remaining four (envelope, barrier, probe, content policy) are the **adaptive monitoring layer**. Behavioral anomaly detection is hard to get right - it requires tuning thresholds to specific workloads, accepting a tradeoff between false positives and missed adversaries, and dealing with failure modes that only surface when many agents with different activity patterns interact over time. The adaptive layer is therefore optional: deployments that don't need runtime anomaly detection can use the static layer alone. All seven checks are wired through the guard's `write_mark()` enforcement boundary and post-write hooks ([spec 8.6](spec.md#86-write-hooks)).

**How the adaptive layer works.** The **statistical envelope** gives each agent its own pluggable anomaly detector (default: `WelfordDetector`) that learns that agent's individual baseline from streaming window counts. No assumption that agents share the same activity pattern. Agents can declare expected activity in their manifest (`expected_activity`) to pre-seed the baseline. The envelope detects rate spikes, type distribution shifts, and cross-agent concentration. Detection algorithms are swappable via the `AnomalyDetector` protocol. See [spec 9.7](spec.md#97-statistical-envelope), properties P40-P43. The **absorbing barrier** narrows an agent's writable mark types and increases need-mark requirements when the envelope flags anomalies. The narrowing is monotonic - scope can shrink but not grow without principal intervention (P44-P45). See [spec 9.8](spec.md#98-absorbing-barrier). The **diagnostic probe** writes a canary mark to the diagnostics scope; the harness reads it back into the system prompt on agent init and gates all tool calls on acknowledgment - an agent that does not call `acknowledge_canary` with the correct ID gets zero actions that round. After the round, non-acknowledgers are marked COMPROMISED: the probe writes a blocking Need for principal review and the guard applies a global barrier revoking observation and action permissions (only Need marks remain writable until the principal restores access). The probe tests whether system prompt instructions survive competing injection through untrusted channels - it catches total behavioral takeover, not targeted adversarial behavior. A knowledgeable adversary that acknowledges the canary and then acts adversarially evades detection. See [spec 9.9](spec.md#99-diagnostic-probe), property P46. **Content policies** provide pluggable content inspection for Observation marks. Policies compose as a pipeline: each policy can ALLOW, REWRITE (sanitize content), REJECT (block the write), or FLAG (allow but warn). Content policies run after rate limits and before scope validation - late in the stack because they may be expensive (LLM calls). They operate independently of the envelope and barrier (P70). See [spec 9.13](spec.md#913-content-policies), properties P67-P70.

**What this catches and what it doesn't.** A prompt-injected agent is structurally equivalent to a low-quality agent: checks are enforced at the mark space boundary, not inside the agent. Neither the static nor the adaptive structural layers catch well-formed lies from authorized agents. Content policies ([spec 9.13](spec.md#913-content-policies)) provide pluggable semantic inspection of Observation content - they can detect and rewrite known-bad patterns (complacent language, fabricated claims, excessive length) or reject observations that violate domain-specific constraints. However, content policies are author-provided checks, not general lie detectors: a well-formed false observation that doesn't match any policy pattern still passes at full trust weight (1.0). If multiple compromised agents write corroborating false observations, the reinforcement mechanism amplifies the signal (two observations: aggregate 1.3; three: 1.39; capped at 2.0). Slow-drift semantic poisoning - gradually shifting observation content without triggering content policy patterns, rate anomalies, or type distribution changes - remains the hardest attack to detect. The probe cannot detect semantic corruption: an agent that acknowledges canaries correctly can still write false observations. The adversarial experiments ([trials analysis](../experiments/trials/analysis.md), [defense trials](../experiments/trials/results/defense/analysis.md)) test the authorization-enforcement surface; the semantic-attack surface is partially addressed by content policies but not comprehensively exercised.
...
**CORAL** ([Human-Agent-Society/CORAL](https://github.com/Human-Agent-Society/CORAL), [Qu et al., 2026](https://arxiv.org/abs/2604.01658)): agents initialized identically run autonomous multi-agent evolution for open-ended discovery, coordinating through a shared filesystem directory rather than messages. Evaluated on 11 benchmark tasks across mathematical optimization (circle packing, signal processing, third autocorrelation inequality, and others) and systems optimization (transaction scheduling, LLM-SQL, and others), plus two additional stress tests (kernel engineering, polyominoes) used for multi-agent evaluation. Agents write typed artifacts - scored attempts, markdown notes, reusable skills - to a symlinked `.coral/public/` directory that all agents read. One agent's discoveries influence others through what it writes, not through direct communication - independent convergence on stigmergic coordination. Despite identical initialization, agents develop differentiated strategies: more than half of each agent's strategy vocabulary is unique. Outperforms fixed-pipeline evolutionary baselines (OpenEvolve, ShinkaEvolve, EvoX) on all 11 tasks, establishing new state-of-the-art on 8. Operates in a cooperative-only setting: no identity enforcement (agent IDs are labels, not cryptographic), no permission model (all agents read/write everything), no conflict resolution (unique filenames avoid collisions), no decay (append-only, a heartbeat prompt asks agents to consolidate but this is not protocol-enforced), and no human escalation (agents are explicitly instructed "do not ask for permission"). The paper's Limitations section flags missing bootstrapped heterogeneity and human-agent negotiation as future work - both addressed by the mark space protocol. CORAL's append-only shared memory grows without bound; the mark space's decay and garbage collection mechanisms bound active state but do not fully solve the accumulation problem (see [Context Accumulation](#context-accumulation)).
...
- [Human-Agent-Society/CORAL](https://github.com/Human-Agent-Society/CORAL). Qu, A. et al. (2026). "CORAL: Towards Autonomous Multi-Agent Evolution for Open-Ended Discovery." [arXiv:2604.01658](https://arxiv.org/abs/2604.01658). [Autonomous multi-agent evolution via shared filesystem artifacts. Independent convergence on stigmergic coordination in a cooperative-only setting.]
