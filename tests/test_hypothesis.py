...
st_confidence = floats(min_value=0.0, max_value=1.0, allow_nan=False, allow_infinity=False)
st_strength = floats(min_value=0.0, max_value=10.0, allow_nan=False, allow_infinity=False)
...
class TestDecayMonotonicity:
    ...
    @given(
        ...
        t1=st_time,
        delta=floats(min_value=0.0, max_value=1e8, allow_nan=False, allow_infinity=False),
    )
...
class TestIntentStepFunction:
    ...
    @given(
        mark=st_intent(),
        config=st_decay_config(),
        extra=floats(min_value=0.001, max_value=1e6, allow_nan=False, allow_infinity=False),
    )
...
class TestNeedResolutionBehavior:
    ...
    def test_unresolved_full_strength(self, config, now, strength):
        mark = Need(scope="test", question="q", initial_strength=strength, created_at=0.0)
...
class TestTrustOrdering:
    ...
    @given(config=st_decay_config(), now=st_time, confidence=st_confidence, strength=st_strength)
    def test_effective_strength_preserves_order(self, config, now, confidence, strength):
        assume(now >= 0.0)
        base = dict(
            scope="test",
            topic="t",
        ...
        fleet = effective_strength(Observation(**base, source=Source.FLEET), now, config)
        verified = effective_strength(
            Observation(**base, source=Source.EXTERNAL_VERIFIED), now, config
        )
        unverified = effective_strength(
            Observation(**base, source=Source.EXTERNAL_UNVERIFIED), now, config
        )
...
class TestWarningInvalidationProperties:
    ...
    def test_recovery_as_warnings_decay(self, config):
        ...
        if base > 0.0:
            suppressed = effective_strength_with_warnings(obs, [warning], t_late, config)
...
class MarkSpaceStateMachine(RuleBasedStateMachine):
    ...
    def resolve_a_need(self):
        ...
        if unresolved:
            nid = unresolved[0]
            self.space.resolve(nid, uuid.uuid4())
